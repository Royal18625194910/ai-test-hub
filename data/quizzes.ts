import { Quiz, Result, MBTIDimensionScores } from './quizzes/types';
import { mbtiQuiz } from './quizzes/mbti';

export * from './quizzes/types';

const personalityTypeQuiz: Quiz = {
  id: 'personality-type',
  title: '你的性格类型测试',
  ...
  quiz.questions.forEach(question => {
    ...
    if (selectedOption) {
      Object.entries(selectedOption.score).forEach(([key, value]) => {
        const dimensionKey = key as keyof MBTIDimensionScores;
        scores[dimensionKey] = (scores[dimensionKey] || 0) + value;
      });
    }
  });
...
function getMBTIType(scores: MBTIDimensionScores): string {
...
export function calculateResult(
  ...
  const calculator = quiz.resultCalculator || 'simple';
  
  if (calculator === 'mbti') {
    const scores = calculateMBTIScores(answers, quiz);
    const mbtiType = getMBTIType(scores);
    
    const result = quiz.results.find(r => r.mbtiType === mbtiType);
...
export function getMBTIDimensionBreakdown(scores: MBTIDimensionScores): {
  E: { score: number; label: string; percentage: number };
  I: { score: number; label: string; percentage: number };
  S: { score: number; label: string; percentage: number };
  N: { score: number; label: string; percentage: number };
  T: { score: number; label: string; percentage: number };
  F: { score: number; label: string; percentage: number };
  J: { score: number; label: string; percentage: number };
  P: { score: number; label: string; percentage: number };
} {
  ...
  return {
    E: { score: scores.E, label: '外向 (Extraversion)', percentage: totalEI > 0 ? Math.round((scores.E / totalEI) * 100) : 50 },
    I: { score: scores.I, label: '内向 (Introversion)', percentage: totalEI > 0 ? Math.round((scores.I / totalEI) * 100) : 50 },
    S: { score: scores.S, label: '感觉 (Sensing)', percentage: totalSN > 0 ? Math.round((scores.S / totalSN) * 100) : 50 },
    N: { score: scores.N, label: '直觉 (Intuition)', percentage: totalSN > 0 ? Math.round((scores.N / totalSN) * 100) : 50 },
    T: { score: scores.T, label: '思考 (Thinking)', percentage: totalTF > 0 ? Math.round((scores.T / totalTF) * 100) : 50 },
    F: { score: scores.F, label: '情感 (Feeling)', percentage: totalTF > 0 ? Math.round((scores.F / totalTF) * 100) : 50 },
    J: { score: scores.J, label: '判断 (Judging)', percentage: totalJP > 0 ? Math.round((scores.J / totalJP) * 100) : 50 },
    P: { score: scores.P, label: '知觉 (Perceiving)', percentage: totalJP > 0 ? Math.round((scores.P / totalJP) * 100) : 50 },
  };
}
