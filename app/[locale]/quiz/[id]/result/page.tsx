                ...
                <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                  {isLoaded && isSignedIn && user ? (
                    <div className="flex items-center gap-1 sm:gap-2">
                      ...
                      <span className="text-xs sm:text-sm text-stone-600 dark:text-stone-400 font-medium truncate max-w-20 sm:max-w-none">
                        {user.emailAddresses[0]?.emailAddress || user.username}
                      </span>
                    </div>
                  ) : (
                    <Link href="/sign-in" className="flex items-center gap-1 sm:gap-2 text-stone-900 dark:text-stone-100 hover:text-orange-600 dark:hover:text-orange-400 transition-colors">
                      <LogIn className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" />
                      <span className="font-bold text-sm hidden sm:inline">{t('common.signIn') || '登录'}</span>
                    </Link>
                  )}
                  <LanguageSwitcher />
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center border-2 border-stone-900 flex-shrink-0">
